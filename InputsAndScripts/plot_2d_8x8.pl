...
p "ThetaPhi_Temp0.0100000000MicroState0.txt" u ($1-($2*0.5)):($2*(sqrt(3)/2.0)):($5*sin($3)*0.5 ):($5*cos($3)*0.5):(cos($3)) w vec lw 5 linecolor palette z
#"/home/nitin/Desktop/Telperion/data/home/n01/Triangular_Lattice_MCMF_Aug17_2020/ZigZagAlongZ_8x8_SIA0.5/U_6.0/ThetaPhi_Temp0.0001000000MicroState0.txt" u ($1-($2*0.5)):($2*(sqrt(3)/2.0)):($5*sin($3)*1 ):($5*cos($3)*1):(cos($3)) w vec lw 5 linecolor palette z
